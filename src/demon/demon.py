...
def compare_and_update_node_data(inc_data):
    ...
    inc_round = inc_data['round']
    if len(node.data) == 0:
        new_time_key = str(node.gossip_counter)
        node.data[new_time_key] = new_data
        return
    latest_entry = max(node.data.keys(), key=int)
    new_time_key = str(node.gossip_counter)
    node.data_flow_per_round.setdefault(node.cycle, {})
    for key in new_data:
        if key in node.data[latest_entry]:
        ...
        if key in node.data[latest_entry] and key in new_data:
            ...
            # Update carbon emission data if available
            if "carbonEmission" in new_data[key]["hbState"]:
                node.data[new_time_key][key]["hbState"]["carbonEmission"] = new_data[key]["hbState"]["carbonEmission"]
            if "carbonEmissionRate" in new_data[key]["hbState"]:
                node.data[new_time_key][key]["hbState"]["carbonEmissionRate"] = new_data[key]["hbState"]["carbonEmissionRate"]
    ...
    if node.is_send_data_back == "1":
        # Include carbon emission data in monitoring update
        carbon_data = node.calculate_carbon_emission()
        to_send['carbon_emission'] = carbon_data
